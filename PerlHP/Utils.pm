package PerlHP::Utils;
require Exporter;
our @ISA=qw(Exporter);
our @EXPORT=qw(abbreviate_html do_wakabamark compile_template process_tripcode
make_date parse_http_date cfg_expand encode_base64 decode_base64 make_random_string
null_string read_array write_array spam_check analyze_image make_thumbnail
rc4 setup_rc6 encrypt_rc6 decrypt_rc6 clean_string decode_string);
...
sub do_wakabamark($$$)
{
	my ($handler,$simplify,$text)=@_;
	my $res;

	$text=clean_string(decode_string($text,'utf8'));
	...
	while(defined($_=$lines[0]))
	{
		...
		elsif(/^&gt;.*/) # quoted sections
		{
			...
			$res.="<blockquote><p>".do_spans($handler,@quote)."</p></blockquote>";
...
sub analyze_svg($)
{
	...
	unless($@)
	{
		...
		return () unless $svg;
		my $elt=$svg->getFirstChild;
		my %attributes=$elt->getAttributes();
